...
# These modules are necessary for SFEE
import binascii
...
class Request:
  def __init__(self, server, cfg):
    ...
    ### -- Begin TeamForge customizations to 'cfg' --

    # Consult the environment to find our root parents.
    self.cfg.general.root_parents = [os.environ.get('SCM_PARENT_PATH') \
                                     + ':' + os.environ.get('SCM_TYPE')]
    
    # Figure out if we are in testmode to see if we need to bypass YUI
    useragent = os.environ.get('HTTP_USER_AGENT', 'unknown')
    self.cfg.general.ctf_testmode = useragent.find("httpunit") != -1 \
                                    and 1 or None

    # Get the app server url
    import SourceForge
    app_server_root_url = SourceForge.get('webserver.root-url', '')
    if len(app_server_root_url) > 0:
      if not app_server_root_url[:-3].endswith("localhost"):
        app_server_root_url = app_server_root_url[:-3]
      else:
        app_server_root_url = ''
  ...
  def run_viewvc(self):
    ...
    self.systemid = None   # The TeamForge external system id
    ...
    if self.rootname:
      ...
      if roottype:
        ...
        self.auth = setup_authorizer(cfg, self.username, self.rootname, self)
        ...
        try:
          if roottype == 'cvs':
            ...
            if sys.platform != "win32" and os.getuid() == 0:
              # For SVN, run as httpd.user (apache)
              import SourceForge
              subversion_user = SourceForge.get('httpd.user', 'apache')
              import pwd
              apache_pw = pwd.getpwnam(subversion_user)
              os.setgid(apache_pw[3])
              os.setuid(apache_pw[2])
    ...
    # Figure out SourceForge external system id
    self.systemid = self.query_dict.get('system')
  ...
  def get_link(self, view_func=None, where=None, pathtype=None, params=None):
    ...
    # add 'system' value to the parameter list
    if (self.systemid is not None
        and not params.has_key('system')):
      params['system'] = self.systemid
...
_legal_params = {
  ...
  # SourceForge additional parameters
  'js'            : None,
  'system'        : None,
  }
...
def setup_authorizer(cfg, username, rootname, request):
  import imp
  ...
  # If using the TeamForge authorizer, create a param for the request object
  if cfg.options.authorizer == 'teamforge':
    params['roottype'] = request.roottype
    params['query_dict'] = request.query_dict
...
def common_template_data(request, revision=None, mime_type=None):
  ...
  data = ezt.TemplateData({
    ...
    # added for SourceForge
    'sf_header': cfg.general.ctf_header,
    'testmode' : cfg.general.ctf_testmode,
    'app_server_root_url': cfg.general.ctf_app_server_root_url,
  })
...
class MarkupPipeWrapper:
  ...
  def __init__(self, fp, pretext=None, posttext=None, htmlize=0):
    ...
    self.fps = []
    if pretext:
      self.fps.append([pretext, 0])
    if fp:
      if multibyte == 1:
        self.fps.append([fpstr, 0])
      else:
        self.fps.append([fp, htmlize])
    if posttext:
      self.fps.append([posttext, 0])
    self.which_fp = 0
...
def view_directory(request):
  ...
  # sfee hack to skip display of directories with large number of entries
  sfee_display = len(file_data)
  sfee_display_max = 10000
  if sfee_display > sfee_display_max:
    skip = 'true'
    file_data = []
  else:
    skip = 'false'
  ...
  data.merge(ezt.TemplateData({
    ...
    #sfee hack
    'skip' : skip,
    'sfee_display' : sfee_display,
    'sfee_display_max' : sfee_display_max,
    #sfee end hack
    }))
...
def list_roots(request):
  ...
  for root in cfg.general.svn_roots.keys():
    auth = setup_authorizer(cfg, request.username, root, request)
    try:
